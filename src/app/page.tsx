...
const TodoList = () => {
  ...
  const [celebrationVisible, setCelebrationVisible] = useState(false); // Declare the state for celebration visibility
  ...
  const handleEdit = (index) => {
    ...
    setTask(tasks[index]); // Set the task in the input field for editing
  };
  ...
  useEffect(() => {
    if (tasks.length === 5) {
      setCelebrationVisible(true); // Show celebration when 5 tasks are added
    }
  }, [tasks.length]);

  const handleContinue = () => {
    setCelebrationVisible(false); // Hide celebration when the button is clicked
  };

  return (
    <>
    <div className="bg-gradient-to-r from-[#A64D79]  to-[#FFCCEA] text-black h-screen flex flex-col items-center justify-start pt-12">
    <h1 className="text-3xl font-bold font-[Spartan] text-white pb-3 -mt-4 text-center items-center"> {/* Added `-mt-4` to move it upwards */}
